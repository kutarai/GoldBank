...
import com.goldbank.app.viewmodel.LoanUiState
...
fun LoanApplyScreen(
    ...
    val assetState by assetViewModel.uiState.collectAsState()

    LaunchedEffect(Unit) { assetViewModel.loadAssets() }
    var selectedCollateralIds by rememberSaveable { mutableStateOf(setOf<String>()) }
        ...
        Column(
            ...
            when (step) {
                0 -> {
                    ...
                    // ── Collateral Assets (optional) ──────────────────────────
                    val collateralAssets = assetState.assets
                    if (collateralAssets.isNotEmpty()) {
                        Text(
                            "Collateral Assets (Optional)",
                        ...
                        Text(
                            "Pledge one or more of your registered assets to back this loan.",
                        ...
                        Card(modifier = Modifier.fillMaxWidth()) {
                            Column(modifier = Modifier.padding(vertical = 4.dp)) {
                                collateralAssets.forEachIndexed { index, asset ->
                                    val isChecked = asset.assetId in selectedCollateralIds
                                    Row(
                                        modifier = Modifier
                                            .fillMaxWidth()
                                            .clickable {
                                                selectedCollateralIds = if (isChecked) {
                                                    selectedCollateralIds - asset.assetId
                                                } else {
                                                    selectedCollateralIds + asset.assetId
                                                }
                                            }
                                        ...
                                        Checkbox(
                                            ...
                                            onCheckedChange = { checked ->
                                                selectedCollateralIds = if (checked) {
                                                    selectedCollateralIds + asset.assetId
                                                } else {
                                                    selectedCollateralIds - asset.assetId
                                                }
                                        ...
                                        Column(modifier = Modifier.weight(1f)) {
                                            ...
                                            Text(
                                                "${asset.quantity} ${asset.unit} · ${asset.currentValueCurrency} ${asset.currentValueAmount} · ${asset.status}",
                                                style = MaterialTheme.typography.labelSmall,
                                                color = MaterialTheme.colorScheme.onSurfaceVariant,
                                            )
                                        }
                                    }
                                    if (index < collateralAssets.lastIndex) HorizontalDivider()
                                }
                            }
                        }
                        Spacer(modifier = Modifier.height(16.dp))
                    ...
                    PinInput(
                        ...
                        onComplete = {
                            viewModel.applyForLoan(
                                ...
                                purpose = purpose,
                                pin = it,
                                collateralAssetIds = selectedCollateralIds.toList(),
                            )
                        },
                    )
                    Spacer(modifier = Modifier.height(24.dp))
