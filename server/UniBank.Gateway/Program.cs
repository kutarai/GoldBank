...
using UniBank.Core.Common.Extensions;
using UniBank.Core.Modules.Accounts.Grpc;
using UniBank.Core.Modules.Admin.Grpc;
using UniBank.Core.Modules.Agents.Grpc;
using UniBank.Core.Modules.BillPay.Grpc;
using UniBank.Core.Modules.KYC.Grpc;
using UniBank.Core.Modules.Merchants.Grpc;
using UniBank.Core.Modules.Payments.Grpc;
...
using UniBank.Reporting;
using UniBank.Reporting.Grpc;
...
try
{
    Log.Information("Starting UniBank API Gateway");
    ...
    builder.Host.UseSerilog((context, services, configuration) =>
        configuration
            ...
            .Enrich.FromLogContext()
            .Enrich.WithProperty("Application", "UniBank.Gateway")
    ...
    // Run
    // ---------------------------------------------------------------------------
    Log.Information("UniBank API Gateway started. gRPC on :1111, HTTP on :1112");
    app.Run();
}
catch (Exception ex)
{
    Log.Fatal(ex, "UniBank API Gateway terminated unexpectedly");
}
finally
{
    Log.CloseAndFlush();
