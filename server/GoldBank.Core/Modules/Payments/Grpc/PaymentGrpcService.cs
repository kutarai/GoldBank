...
using GoldBank.Core.Modules.Payments.Application.Commands;
using GoldBank.Core.Modules.Payments.Application.Handlers;
using GoldBank.Protos.Payments;
using GoldBank.SharedKernel.MultiTenancy;

namespace GoldBank.Core.Modules.Payments.Grpc;
...
public sealed class PaymentGrpcService : PaymentService.PaymentServiceBase
{
    ...
    private static PaymentResponse MapToPaymentResponse(PaymentResult value, string currency)
    {
        var response = new PaymentResponse
        {
            ...
            Tax = new GoldBank.Protos.Common.Money
            {
                Amount = value.Tax.ToString("F2"),
                Currency = currency
            },
            MerchantCommission = new GoldBank.Protos.Common.Money
            {
