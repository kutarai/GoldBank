...
public sealed class AssetReleaseHandler
{
    ...
    public AssetReleaseHandler(GoldBankDbContext db, ILogger<AssetReleaseHandler> logger)
    {
        _db = db;
        _logger = logger;
    }
    ...
    /// <summary>
    /// Validates that the asset belongs to the given customer and is currently
    /// <see cref="AssetStatus.Active"/>, then marks it as
    /// <see cref="AssetStatus.PendingRelease"/> pending admin approval.
    /// </summary>
    /// <param name="customerId">The customer who owns the asset.</param>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (asset is null)
            return Result.Failure(new Error(
                ...
                $"Asset {assetId} not found for customer {customerId}."));
