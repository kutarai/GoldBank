...
using GoldBank.Protos.Assets;
using GoldBank.SharedKernel.MultiTenancy;

namespace GoldBank.Core.Modules.AssetCustody.Grpc;
...
///
/// Multi-tenancy is enforced at the database schema level by GoldBankDbContext, so EF Core
...
public sealed class AssetGrpcService : AssetService.AssetServiceBase
{
        ...
        RegisterAssetRequest request, ServerCallContext context)
    {
        ...
        if (!Guid.TryParse(request.CustomerId, out var customerId))
            throw new RpcException(new Status(StatusCode.InvalidArgument, "Valid customer_id is required."));
        ...
        _logger.LogInformation(
            "Asset registered: {AssetId} for customer {CustomerId}, type {AssetType}, receipt {Receipt}",
            asset.Id, customerId, assetType, request.ReceiptNumber);
        ...
        GetAssetDetailRequest request, ServerCallContext context)
    {
        ...
        var asset = await _db.Assets
            ...
            .FirstOrDefaultAsync(
                a => a.Id == assetId && a.CustomerId == customerId && !a.IsDeleted,
                context.CancellationToken);
        ...
        var response = new AssetDetailResponse
        {
            ...
            CurrentValue = new GoldBank.Protos.Common.Money
            {
        ...
        foreach (var v in asset.Valuations)
        {
            ...
            {
                Id = v.Id.ToString(),
                Amount = new GoldBank.Protos.Common.Money
                {
    ...
    public override async Task<GoldBank.Protos.Common.StatusResponse> RequestAssetRelease(
        RequestAssetReleaseRequest request, ServerCallContext context)
    {
        ...
        _logger.LogInformation(
            "Asset release requested: {AssetId} by customer {CustomerId}. Reason: {Reason}",
            assetId, customerId, request.Reason);

        return new GoldBank.Protos.Common.StatusResponse
        {
            Success = true,
        ...
        GetPortfolioValueRequest request, ServerCallContext context)
    {
        ...
        {
            TotalValueZwg = new GoldBank.Protos.Common.Money
            {
                Amount = totalZwg.ToString("F2"),
                Currency = "ZWG",
            },
            TotalValueUsd = new GoldBank.Protos.Common.Money
            {
        ...
        foreach (var (typeName, (count, total)) in byType)
        {
            response.AssetsByType.Add(new AssetTypeSummary
            {
                AssetType = typeName,
                Count = count,
                TotalValue = new GoldBank.Protos.Common.Money
                {
        ...
        SubmitValuationRequest request, ServerCallContext context)
    {
        ...
        return new ValuationResponse
        {
            ...
            NewValue = new GoldBank.Protos.Common.Money
            {
    ...
    public override async Task<GoldBank.Protos.Common.StatusResponse> ApproveAssetRelease(
