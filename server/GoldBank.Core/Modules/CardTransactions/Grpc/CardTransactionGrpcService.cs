...
using GoldBank.Core.Modules.CardTransactions.Application.Handlers;
using GoldBank.Core.Modules.CardTransactions.Application.Validators;
using GoldBank.Protos.CardTransactions;
using GoldBank.SharedKernel.MultiTenancy;

namespace GoldBank.Core.Modules.CardTransactions.Grpc;
...
public sealed class CardTransactionGrpcService : CardTransactionService.CardTransactionServiceBase
{
        ...
        BalanceEnquiryRequest request, ServerCallContext context)
    {
        ...
        return new BalanceEnquiryResponse
        {
            ...
            AvailableBalance = new GoldBank.Protos.Common.Money
            {
                Amount = value.AvailableBalance.ToString("F2"),
                Currency = value.Currency
            },
            LedgerBalance = new GoldBank.Protos.Common.Money
            {
        ...
        StatementEnquiryRequest request, ServerCallContext context)
    {
        ...
        foreach (var entry in value.Entries)
        {
            response.Entries.Add(new StatementEntry
            {
                ...
                BalanceAfter = new GoldBank.Protos.Common.Money
                {
    ...
    private static decimal ParseMoney(GoldBank.Protos.Common.Money money)
    {
    ...
    private static CardTransactionResponse MapFailureToResponse(
        GoldBank.SharedKernel.Results.Error error, string transactionId, string currency)
    {
