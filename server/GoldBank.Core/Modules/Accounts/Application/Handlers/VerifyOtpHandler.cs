...
using GoldBank.Core.Modules.Accounts.Domain.ValueObjects;
...
public sealed class VerifyOtpHandler
{
        ...
        VerifyOtpCommand command, CancellationToken cancellationToken = default)
    {
        ...
        // Create or reuse the Customer (one per tenant + phone)
        var customer = await _dbContext.Customers
            .FirstOrDefaultAsync(
                c => c.TenantId == command.TenantId && c.PhoneNumber == phone.Value && c.DeletedAt == null,
                cancellationToken);
        ...
        // Create dual-currency accounts (ZWG + USD) linked to the Customer
