...
using GoldBank.Core.Modules.Loans.Application.Handlers;
using GoldBank.Protos.Loans;
using GoldBank.SharedKernel.MultiTenancy;

namespace GoldBank.Core.Modules.Loans.Grpc;
...
public sealed class LoanGrpcService : LoanService.LoanServiceBase
{
        ...
        ApplyForLoanRequest request, ServerCallContext context)
    {
        ...
        var collateralIds = new List<Guid>(request.CollateralAssetIds.Count);
        foreach (var raw in request.CollateralAssetIds)
        {
            if (!Guid.TryParse(raw, out var assetGuid))
                throw new RpcException(new Status(
                    StatusCode.InvalidArgument,
                    $"Invalid collateral asset id: {raw}"));
            collateralIds.Add(assetGuid);
        }

        _logger.LogInformation(
            "ApplyForLoan request: account {AccountId}, amount {Amount} {Currency}, tenure {Tenure}mo, collateral {CollateralCount}",
            accountId, amount, currency, request.TenureMonths, collateralIds.Count);
        ...
        var command = new ApplyForLoanCommand(
            ...
            Pin: request.Pin,
            TenantId: tenantId,
            CollateralAssetIds: collateralIds);
        ...
        return new LoanApplicationResponse
        {
            ...
            Principal = new GoldBank.Protos.Common.Money { Amount = v.Principal.ToString("F2"), Currency = v.Currency },
            InterestRate = v.InterestRate.ToString("P2"),
            MonthlyPayment = new GoldBank.Protos.Common.Money { Amount = v.MonthlyPayment.ToString("F2"), Currency = v.Currency },
            TenureMonths = v.TenureMonths,
            CreditScore = v.CreditScore,
            NewBalance = new GoldBank.Protos.Common.Money { Amount = v.NewBalance.ToString("F2"), Currency = v.Currency },
        };
    }
        ...
        GetLoanRequest request, ServerCallContext context)
    {
        ...
        var response = new LoanDetailResponse
        {
            ...
            Principal = new GoldBank.Protos.Common.Money { Amount = loan.Principal.ToString("F2"), Currency = loan.Currency },
            OutstandingBalance = new GoldBank.Protos.Common.Money { Amount = loan.OutstandingBalance.ToString("F2"), Currency = loan.Currency },
            InterestRate = loan.InterestRate.ToString("P2"),
            TenureMonths = loan.TenureMonths,
            MonthlyPayment = new GoldBank.Protos.Common.Money { Amount = loan.MonthlyPayment.ToString("F2"), Currency = loan.Currency },
        ...
        GetLoanScheduleRequest request, ServerCallContext context)
    {
        ...
        foreach (var p in result.Value)
        {
            ...
            response.Entries.Add(new LoanScheduleEntry
            {
                ...
                PrincipalAmount = new GoldBank.Protos.Common.Money { Amount = p.PrincipalAmount.ToString("F2"), Currency = currency },
                InterestAmount = new GoldBank.Protos.Common.Money { Amount = p.InterestAmount.ToString("F2"), Currency = currency },
                TotalPayment = new GoldBank.Protos.Common.Money { Amount = p.TotalPayment.ToString("F2"), Currency = currency },
                RemainingBalance = new GoldBank.Protos.Common.Money { Amount = p.RemainingBalance.ToString("F2"), Currency = currency },
    ...
    private static StatusCode MapErrorToStatusCode(string errorCode)
    {
        return errorCode switch
        {
            ...
            "Loan.InvalidCollateral" => StatusCode.InvalidArgument,
