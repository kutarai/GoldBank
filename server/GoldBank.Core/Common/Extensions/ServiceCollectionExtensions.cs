...
using GoldBank.Core.Common.Caching;
using GoldBank.Core.Common.Messaging;
...
using GoldBank.Core.Modules.Accounts.Application.Validators;
...
namespace GoldBank.Core.Common.Extensions;
...
public static class ServiceCollectionExtensions
{
    public static IServiceCollection AddCoreModules(this IServiceCollection services, IConfiguration configuration)
    {
        var connectionString = configuration.GetConnectionString("DefaultConnection")
            ?? "Host=localhost;Database=goldbank;Username=goldbank;Password=goldbank_dev_password";

        services.AddDbContext<PublicDbContext>(options =>
            options.UseNpgsql(connectionString));

        services.AddDbContext<GoldBankDbContext>(options =>
            options.UseNpgsql(connectionString));
        ...
        // AssetGrpcService depends only on GoldBankDbContext, ITenantProvider, and ILogger —
