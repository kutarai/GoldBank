...
using UniBank.Core.Modules.AssetCustody.Application.Commands;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            _logger.LogInformation(
                "Deposit receipt OCR completed for account {AccountId}: receipt={Receipt}, house={House}",
                command.AccountId, fields.ReceiptNumber, fields.DepositHouse);
        ...
        {
            _logger.LogError(ex,
                "Deposit receipt OCR failed for account {AccountId}", command.AccountId);
