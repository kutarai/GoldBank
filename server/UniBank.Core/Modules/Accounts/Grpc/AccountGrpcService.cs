...
namespace UniBank.Core.Modules.Accounts.Grpc;
...
public sealed class AccountGrpcService : AccountService.AccountServiceBase
{
        ...
        GetBalanceRequest request, ServerCallContext context)
    {
        ...
        return new BalanceResponse
        {
            ...
            Balance = new UniBank.Protos.Common.Money { Amount = b.Balance.ToString("F2"), Currency = b.Currency },
            AvailableBalance = new UniBank.Protos.Common.Money { Amount = b.AvailableBalance.ToString("F2"), Currency = b.Currency },
            DailyLimit = new UniBank.Protos.Common.Money { Amount = b.DailyLimit.ToString("F2"), Currency = b.Currency }
        };
    }
        ...
        ServerCallContext context)
    {
        ...
        foreach (var tx in result.Value)
        {
            await responseStream.WriteAsync(new TransactionResponse
            {
                ...
                Amount = new UniBank.Protos.Common.Money { Amount = tx.Amount.ToString("F2"), Currency = tx.Currency },
                Fee = new UniBank.Protos.Common.Money { Amount = tx.Fee.ToString("F2"), Currency = tx.Currency },
                ...
                BalanceAfter = new UniBank.Protos.Common.Money { Amount = tx.BalanceAfter.ToString("F2"), Currency = tx.Currency },
