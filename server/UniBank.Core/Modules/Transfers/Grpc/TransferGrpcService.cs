...
using UniBank.Core.Modules.Transfers.Application.Commands;
using UniBank.Core.Modules.Transfers.Application.Handlers;
using UniBank.Protos.Transfers;
using UniBank.SharedKernel.MultiTenancy;

namespace UniBank.Core.Modules.Transfers.Grpc;
...
public sealed class TransferGrpcService : TransferService.TransferServiceBase
{
    ...
    private static TransferResponse MapToTransferResponse(TransferResult value)
    {
        var response = new TransferResponse
        {
            ...
            AmountSent = new UniBank.Protos.Common.Money
            {
                Amount = value.AmountSent.ToString("F2"),
                Currency = value.Currency
            },
            AmountReceived = new UniBank.Protos.Common.Money
            {
