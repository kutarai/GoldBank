...
/// <summary>
/// Seeds the GoldBank database with demo data that matches the bank-client React app's
...
internal static class DemoSeeder
{
    ...
    private static Guid CustomerUuid(int row) => MakeUuid(0x0e, row);
    private static Guid EkubGroupUuid(int row) => MakeUuid(0x0f, row);
    private static Guid EkubMembershipUuid(int row) => MakeUuid(0x10, row);
    private static Guid EkubContributionUuid(int row) => MakeUuid(0x11, row);
    private static Guid EkubLoanUuid(int row) => MakeUuid(0x12, row);
    private static Guid EkubLoanVoteUuid(int row) => MakeUuid(0x13, row);
    ...
    public static async Task SeedAsync(GoldBankDbContext db)
    {
        ...
        await BackfillAccountCustomerLinksAsync(db);
        await BackfillDemoAccountPinHashesAsync(db);
    ...
    private static async Task SeedAdminUsersAsync(GoldBankDbContext db)
    {
        ...
        var users = new[]
        {
            ...
            New<AdminUser>(AdminUuid(4), u =>
            {
                ...
                u.Email = "support@goldbank.co.zw";
            ...
            New<AdminUser>(AdminUuid(6), u =>
            {
                ...
                u.Email = "compliance@goldbank.co.zw";
    ...
    private static async Task BackfillAdminPasswordHashesAsync(GoldBankDbContext db)
    {
    ...
    private static async Task EnsureTellerUserAsync(GoldBankDbContext db)
    {
    ...
    // ── Customers ──────────────────────────────────────────────────────────
    // 20 customer rows (one per phone). Idempotent: skipped if rows already exist.
    // Demo customer phones use the same data list as SeedAccountsAsync below.

    private static async Task SeedCustomersAsync(GoldBankDbContext db)
    {
        if (await db.Customers.CountAsync() >= 20) return;

        var customers = DemoCustomers
            .Select((c, i) => New<Customer>(CustomerUuid(i + 1), x =>
            {
                x.PhoneNumber = c.phone;
                x.PhoneCountryCode = "+263";
                x.FirstName = c.fn;
                x.LastName = c.ln;
                x.Email = $"{c.fn.ToLower()}@email.co.zw";
                x.NationalId = c.nationalId;
                x.TenantId = "goldbank";
                x.Status = "active";
                x.CreatedAt = Ago(c.createdDays);
            }))
            .ToList();

        db.Customers.AddRange(customers);
        await db.SaveChangesAsync();
        Console.WriteLine($"  [Customers] seeded {customers.Count}");
    }

    // Idempotent: replace any placeholder PIN hashes on demo customer accounts
    // with a real BCrypt hash for "1234". Safe to run repeatedly.
    private static async Task BackfillDemoAccountPinHashesAsync(GoldBankDbContext db)
    {
        var demoPhones = DemoCustomers.Select(c => c.phone).ToHashSet();

        var accounts = await db.Accounts
            .Where(a => demoPhones.Contains(a.PhoneNumber) && a.DeletedAt == null)
            .ToListAsync();

        var realHash = BCrypt.Net.BCrypt.HashPassword("1234", 11);
        var patched = 0;
        foreach (var acct in accounts)
        {
            // Only patch when the existing hash does NOT verify against "1234".
            bool currentlyValid;
            try { currentlyValid = !string.IsNullOrEmpty(acct.PinHash) && BCrypt.Net.BCrypt.Verify("1234", acct.PinHash); }
            catch { currentlyValid = false; }
            if (currentlyValid) continue;

            acct.PinHash = realHash;
            patched++;
        }
        ...
        {
            await db.SaveChangesAsync();
            Console.WriteLine($"  [Accounts] backfilled real BCrypt PIN hash (\"1234\") for {patched} demo customer accounts");
        }
    }

    // Idempotent re-link: ensures every demo account row has its CustomerId set,
    // even when run against a DB that was seeded prior to the Customer aggregate.
    private static async Task BackfillAccountCustomerLinksAsync(GoldBankDbContext db)
    {
        var customersByPhone = await db.Customers
            .ToDictionaryAsync(c => c.PhoneNumber);

        var changed = 0;
        var accounts = await db.Accounts.ToListAsync();
        foreach (var acct in accounts)
        {
            if (acct.CustomerId != Guid.Empty) continue;
            if (customersByPhone.TryGetValue(acct.PhoneNumber, out var customer))
            {
                acct.CustomerId = customer.Id;
                changed++;
            }
        }
        ...
        {
            await db.SaveChangesAsync();
            Console.WriteLine($"  [Accounts] back-linked {changed} accounts to their customers");
        }
    }
    ...
    // Demo customers — used by both SeedCustomersAsync (for Customer rows) and
    // SeedAccountsAsync (which fans them out into ZWG + USD accounts).
    private static readonly (string fn, string ln, string phone, string status, int kycLevel,
        decimal balZwg, decimal balUsd, int createdDays, int lastLoginDays, string nationalId)[] DemoCustomers = new[]
    {
    ...
    private static async Task SeedAccountsAsync(GoldBankDbContext db)
    {
        ...
        {
            var (fn, ln, phone, status, kycLevel, balZwg, balUsd, createdDays, loginDays, nationalId) = DemoCustomers[i];
            var customerId = CustomerUuid(i + 1);
            var email = $"{fn.ToLower()}@email.co.zw";
            // Demo PIN is "1234" — verifies via PinHashingService (BCrypt). Same for every demo customer.
            var pin = BCrypt.Net.BCrypt.HashPassword("1234", 11);
    ...
    private static async Task SeedTransactionsAsync(GoldBankDbContext db)
    {
    ...
    private static async Task SeedDisputesAsync(GoldBankDbContext db)
    {
    ...
    private static async Task SeedFraudAlertsAsync(GoldBankDbContext db)
    {
    ...
    private static async Task SeedKycDocumentsAsync(GoldBankDbContext db)
    {
    ...
    private static async Task SeedLoansAsync(GoldBankDbContext db)
    {
    ...
    private static async Task SeedBranchesAsync(GoldBankDbContext db)
    {
    ...
    private static async Task SeedMerchantsAsync(GoldBankDbContext db)
    {
    ...
    private static async Task SeedBillProvidersAsync(GoldBankDbContext db)
    {
    ...
    private static async Task SeedSystemConfigsAsync(GoldBankDbContext db)
    {
        ...
        var rows = new (string key, string value)[]
        {
            ...
            ("ai.ollama_url",                   "\"http://goldbank-ollama:11434\""),
            ("ai.model_name",                   "\"qwen3-vl\""),
            ("ekub.monthly_fee_zwg",            "\"50.00\""),
            ("ekub.monthly_fee_usd",            "\"2.00\""),
        };
    ...
    // ── 12. Ekub demo group ───────────────────────────────────────────────
    // One Active group "Borrowdale Savings" chaired by Tendai Moyo (customer 1),
    // with Treasurer + Secretary roles assigned and three pre-recorded
    // contributions (two confirmed, one pending) so the mobile UI has data
    // immediately. If John Moyo (+263771000001) exists, he joins as a member
    // so the registered demo phone has an active group on first launch.

    private static async Task SeedEkubAsync(GoldBankDbContext db)
    {
        if (await db.EkubGroups.AnyAsync()) return;

        // Resolve customer IDs by phone — the migration backfills customers with
        // gen_random_uuid(), so the deterministic CustomerUuid(n) values aren't
        // valid customer IDs. Phone is the canonical natural key.
        async Task<Guid?> CustomerIdForPhoneAsync(string phone) =>
            (await db.Customers.FirstOrDefaultAsync(c => c.PhoneNumber == phone && c.DeletedAt == null))?.Id;

        var chairman   = await CustomerIdForPhoneAsync("+263770003287") ?? Guid.Empty; // Tendai Moyo
        var treasurer  = await CustomerIdForPhoneAsync("+263775304489") ?? Guid.Empty; // Chiedza Mutasa
        var secretary  = await CustomerIdForPhoneAsync("+263771882741") ?? Guid.Empty; // Farai Chikwanha
        var member4    = await CustomerIdForPhoneAsync("+263774538185") ?? Guid.Empty; // Nyasha Dube

        if (chairman == Guid.Empty || treasurer == Guid.Empty || secretary == Guid.Empty)
        {
            Console.WriteLine("  [Ekub] skipped — demo customers not found by phone (run accounts seed first)");
            return;
        }

        var groupId    = EkubGroupUuid(1);
        var tenantId   = "goldbank";
        var now        = Now;
        var period     = now.ToString("yyyy-MM");

        var group = New<EkubGroup>(groupId, g =>
        {
            g.Name = "Borrowdale Savings";
            g.Description = "Monthly savings group for Borrowdale residents.";
            g.Currency = "USD";
            g.MonthlyContribution = 50.00m;
            g.LoanInterestRatePercent = 8.000m;
            g.Status = EkubGroupStatus.Active;
            g.ChairmanCustomerId = chairman;
            g.TenantId = tenantId;
            g.ActivatedAt = Ago(60);
            g.CreatedAt   = Ago(75);
        });
        db.EkubGroups.Add(group);

        var memberships = new List<(Guid customerId, EkubMemberRole role, int membershipRow, int joinedDaysAgo)>
        {
            (chairman,  EkubMemberRole.Chairman,  1, 75),
            (treasurer, EkubMemberRole.Treasurer, 2, 65),
            (secretary, EkubMemberRole.Secretary, 3, 60),
            (member4,   EkubMemberRole.Member,    4, 45),
        };

        // Optional: add John Moyo (+263771000001) as a Member if his customer row exists.
        var john = await db.Customers
            .FirstOrDefaultAsync(c => c.PhoneNumber == "+263771000001" && c.DeletedAt == null);
        if (john is not null)
            memberships.Add((john.Id, EkubMemberRole.Member, 5, 30));

        foreach (var (cid, role, row, daysAgo) in memberships)
        {
            db.EkubMemberships.Add(New<EkubMembership>(EkubMembershipUuid(row), m =>
            {
                m.GroupId = groupId;
                m.CustomerId = cid;
                m.Role = role;
                m.JoinedAt = Ago(daysAgo);
                m.TenantId = tenantId;
                m.CreatedAt = Ago(daysAgo);
            }));
        }

        // Three contributions for the current period, two confirmed.
        var contributions = new (int row, Guid memberCustomerId, int membershipRow, decimal amount, EkubContributionStatus status, int daysAgo)[]
        {
            (1, chairman,  1, 50.00m, EkubContributionStatus.Confirmed, 12),
            (2, treasurer, 2, 50.00m, EkubContributionStatus.Confirmed, 10),
            (3, secretary, 3, 50.00m, EkubContributionStatus.Pending,    2),
        };

        foreach (var (row, cid, membershipRow, amount, status, daysAgo) in contributions)
        {
            db.EkubContributions.Add(New<EkubContribution>(EkubContributionUuid(row), c =>
            {
                c.GroupId = groupId;
                c.CustomerId = cid;
                c.MembershipId = EkubMembershipUuid(membershipRow);
                c.Amount = amount;
                c.Currency = "USD";
                c.Period = period;
                c.Status = status;
                c.ConfirmedByCustomerId = status == EkubContributionStatus.Confirmed ? treasurer : null;
                c.ConfirmedAt           = status == EkubContributionStatus.Confirmed ? Ago(daysAgo) : null;
                c.TenantId = tenantId;
                c.CreatedAt = Ago(daysAgo);
            }));
        }

        // Sample loan in Voting state so the voting UI is exercisable from
        // first launch. Borrower is John (member4 — Nyasha Dube — if John was
        // unavailable). Includes one approve vote from the chairman so the UI
        // shows partial-progress.
        var borrowerId = john?.Id ?? member4;
        var loanId     = EkubLoanUuid(1);
        var loanRate   = 8.000m;
        var principal  = 200.00m;
        var termMonths = 6;
        var totalInterest  = principal * (loanRate / 100m) * termMonths / 12m;
        var totalRepayable = principal + totalInterest;
        var installment    = totalRepayable / termMonths;

        db.EkubLoans.Add(New<EkubLoan>(loanId, l =>
        {
            l.GroupId             = groupId;
            l.BorrowerCustomerId  = borrowerId;
            l.Principal           = principal;
            l.InterestRatePercent = loanRate;
            l.TermMonths          = termMonths;
            l.TotalRepayable      = totalRepayable;
            l.InstallmentAmount   = installment;
            l.OutstandingBalance  = totalRepayable;
            l.TotalInterestEarned = 0m;
            l.Currency            = "USD";
            l.Status              = EkubLoanStatus.Voting;
            l.Purpose             = "School fees for Q3";
            l.TenantId            = tenantId;
            l.CreatedAt           = AgoH(8);
        }));

        // One approve vote in the bag — chairman supports.
        db.EkubLoanVotes.Add(New<EkubLoanVote>(EkubLoanVoteUuid(1), v =>
        {
            v.LoanId            = loanId;
            v.VoterCustomerId   = chairman;
            v.Approve           = true;
            v.TenantId          = tenantId;
            v.CreatedAt         = AgoH(6);
        }));
        ...
        await db.SaveChangesAsync();
        Console.WriteLine($"  [Ekub] seeded 1 active group with {memberships.Count} members, 3 contributions and 1 loan in Voting state ({(john is null ? "John not seeded" : "incl. John Moyo")})");
    }

    // ── 11. Audit Logs ────────────────────────────────────────────────────
    // 60 rows (seed=66)

    private static async Task SeedAuditLogsAsync(GoldBankDbContext db)
    {
